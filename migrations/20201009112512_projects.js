...
exports.up = function(knex) {
  return knex.schema.createTable(`projects`, tbl => {
    ...
    .createTable(`tasks`, tbl => {
        ...
        tbl.text(`notes`)
        tbl.integer(`projects_id`).notNullable().unsigned().onUpdate(`CASCADE`).references(`id`).inTable(`projects`)
    })
    .createTable(`projects_resources`, tbl => {
        tbl.integer(`projects_id`).notNullable().unsigned().onUpdate(`CASCADE`).references(`id`).inTable(`projects`)
        tbl.integer(`resources_id`).notNullable().unsigned().onUpdate(`CASCADE`).references(`id`).inTable(`projects`)
